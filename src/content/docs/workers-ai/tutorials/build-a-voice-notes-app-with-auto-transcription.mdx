...
<PackageManagers
	...
	args={"voice-notes --framework=nuxt --experimental"}
/>
...
Once the `AI` binding has been configured, run the `cf-typegen` command to generate the necessary Cloudflare type definitions. This makes the types definitions available in the server request contexts.
	...
	<div class="flex flex-col gap-y-5">
		...
		>
			...
			>
				<template #header>
					<h3
						class="text-base font-medium text-gray-600 dark:text-gray-300 md:text-lg"
					>
						Note transcript
					</h3>
				</template>
...
<template>
	<UModal v-model="isOpen" fullscreen>
		...
		>
			<template #header>
				<h2 class="text-xl font-semibold leading-6 md:text-2xl">Create note</h2>
				<UButton
					color="gray"
					variant="ghost"
...
<template>
	<UCard>
		<template #header>
			<div>
				<h2 class="text-base font-semibold leading-6 md:text-lg">
					Post Processing
				</h2>
