...
The command above will output the ID of your Hyperdrive, which you will need to set in the `wrangler.toml / wrangler.json(c)` file for your Workers project:
...
**Ensure you have `compatibility_flags` and `compatibility_date` set in your `wrangler.toml / wrangler.json(c)` file** as shown below:
