...
You add a Service binding by modifying the `wrangler.toml / wrangler.json(c)` file of the caller — the Worker that you want to be able to initiate requests.

For example, if you want Worker A to be able to call Worker B — you'd add the following to the `wrangler.toml / wrangler.json(c)` file for Worker A:
