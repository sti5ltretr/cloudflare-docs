...
* `wrangler route` - Routes are defined in the `wrangler.toml / wrangler.json(c)` file.
...
These are new fields that can be added to your `wrangler.toml / wrangler.json(c)` file.
...
Routes are specified in the `wrangler.toml / wrangler.json(c)` file.
