...
* `bucket` required
  ...
  * The directory containing your static assets, path relative to your `wrangler.toml / wrangler.json(c)` file. Example: `bucket = "./public"`.
...
Example of a `wrangler.toml / wrangler.json(c)` file:
