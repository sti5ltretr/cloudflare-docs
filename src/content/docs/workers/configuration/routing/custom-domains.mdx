...
### Set up a Custom Domain in your `wrangler.toml / wrangler.json(c)` file

To configure a Custom Domain in your `wrangler.toml / wrangler.json(c)` file, add the `custom_domain=true` option on each pattern under `routes`. For example, to configure a Custom Domain:
...
To migrate the route `example.com/*` in your `wrangler.toml / wrangler.json(c)` file:
